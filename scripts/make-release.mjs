...
/** 写成 Windows 换行（CRLF）：给会在 Windows 记事本 / cmd 里打开的文本用 */
...
const start = (port, tries = 0) => {
  ...
  server.listen(port, '127.0.0.1', () => {
    ...
    console.log(\`  [OK] demo server ready: \${url}\`)
...
四、常见问题
  ...
  * 双击后窗口一闪而过、或提示找不到 dist\\index.html：
      多半是**没解压就直接在压缩包预览里双击**了。Windows 会把压缩包里的文件释放到临时目录再运行，
      这时同目录的 dist 与 node 都不在 → 请先「解压到文件夹」，再到解压出来的目录里双击。
      窗口里出现「[OK] demo server ready: http://127.0.0.1:4188」这一行（纯英文，不受字体影响）就算启动成功。
...
const makeDemoPackage = async () => {
  ...
  // Windows 的 cmd 解析 .bat 时对纯 LF 换行有已知的边界问题（尤其是跨行的 if ( … ) 块），
  // 而这一环在 macOS 上无法真机验证 —— 所以统一按 CRLF 写，并把断言放在这里，宁可打包失败也不要概率性故障。
  await writeFile(join(dir, 'start-demo.bat'), toCrlf(BAT_SOURCE))
  await writeFile(join(dir, 'README.txt'), toCrlf(README_SOURCE))
  for (const f of ['start-demo.bat', 'README.txt', 'node/README.txt']) {
    const p = join(dir, f)
    if (!existsSync(p)) continue
    const bytes = await readFile(p)
    if (!bytes.includes(Buffer.from('\r\n')) || /[^\r]\n/.test(bytes.toString('utf8'))) {
      throw new Error(`${f} 不是纯 CRLF 换行 —— Windows 记事本/cmd 上可能出问题，拒绝打包`)
    }
  }
  if (WITH_NODE) {
...
const makeSourcePackage = async () => {
  ...
  await writeFile(join(dir, '安装与启动.md'), toCrlf(SRC_README))

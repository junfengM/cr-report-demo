...
const download = {
  // 通用文件下载：MIME 优先用调用方指定，其次用 Blob 自带的 type（如报文 .txt/.csv/.xml）
  file: (data: Blob, fileName: string, mineType?: string) =>
    download0(data, fileName, mineType || data?.type || 'application/octet-stream'),
  // 下载 Excel 方法
